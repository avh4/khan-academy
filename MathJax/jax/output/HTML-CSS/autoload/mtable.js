...
(function (MML,HTMLCSS) {
  ...
  MML.mtable.Augment({
    toHTML: function (span) {
      ...
      var H = [], D = [], W = [], A = [], C = [], RCALIGN = [], i, j, J = -1, m, M, s, row;
      ...
      //
      var y = Y, dx, dy, line, align; s = (C[LABEL] ? LABEL : 0);
      for (j = s; j <= J; j++) {
        for (i = 0, m = A.length; i < m; i++) {
          if (A[i][j]) {
      ...
      if (hasRelativeWidth) {
        ...
        for (j = 0; j <= J; j++) {
          ...
          //
          C[j].style.position = "relative"; C[j].style.width = "100%";
          C[j].style.left = HTMLCSS.Em(xf); var top = C[j].parentNode.style.top;
