var KnowledgeMapAdmin = {
    ...
    options: {
                getTileUrl: function(coord, zoom) {
                    ...
                    return KnowledgeMapAdmin.getHorizontallyRepeatingTileUrl(coord, zoom, 
                            function(coord, zoom) {
    ...
    init: function(latInit, lngInit, zoomInit) {
        ...
        google.maps.event.addListener(this.map, "center_changed", function(){KnowledgeMapAdmin.onCenterChange();});
        google.maps.event.addListener(this.map, "idle", function(){KnowledgeMapAdmin.onIdle();});
    ...
    discoverGraph: function() {
        ...
        $("table.hidden_knowledge_map tr[data-id]").each(function(){
            ...
            jel.find("li[data-prereq]").each(function(i) {
                ...
                KnowledgeMapAdmin.addEdge(source, target, summative);
            });
        });
    },
    ...
    drawOverlay: function() {
        ...
        this.overlay.draw = function() {
            ...
            if (!KnowledgeMapAdmin.fFirstDraw)
            {
                KnowledgeMapAdmin.onZoomChange(jrgNodes);
            }

            jrgNodes.each(function(){
                KnowledgeMapAdmin.attachNodeEvents(this, KnowledgeMapAdmin.dictNodes[$(this).attr("data-id")]);
            });

            KnowledgeMapAdmin.fFirstDraw = false;
        }
    },

    attachNodeEvents: function(el, node) {
        $(el).click(
                function(){KnowledgeMapAdmin.onNodeClick(node);}
            ).hover(
                function(){KnowledgeMapAdmin.onNodeMouseover(this, node);},
                function(){KnowledgeMapAdmin.onNodeMouseout(this, node);}
            );
    },
    ...
    highlightNode: function(node, highlight) {
        var jel = $("#node-" + KnowledgeMapAdmin.escapeSelector(node.id));
        if (highlight)
    ...
    onNodeClick: function(node) {
        ...
        $(document).keydown(function(e){
            ...
            if ( 37 <= e.keyCode && e.keyCode <= 40 ) {
                var zoom =KnowledgeMapAdmin.map.getZoom();
                KnowledgeMapAdmin.markers = [];

                for (var key in KnowledgeMapAdmin.dictEdges)
                    {
                        var rgTargets = KnowledgeMapAdmin.dictEdges[key];
                        for (var ix = 0; ix < rgTargets.length; ix++)
                        {
                            rgTargets[ix].line.setMap(null);
                        }
                    }
                KnowledgeMapAdmin.dictNodes[node.id] = node;
                KnowledgeMapAdmin.overlay.setMap(null);
                KnowledgeMapAdmin.layoutGraph();
                KnowledgeMapAdmin.drawOverlay();
                return false;
           }
        });
    ...
    onNodeMouseover: function(el, node) {
        ...
        $(".exercise-edit[data-id=\"" + KnowledgeMapAdmin.escapeSelector(node.id) + "\"]").addClass("exercise-edit-hover");
    ...
    onNodeMouseout: function(el, node) {
        ...
        $(".exercise-edit[data-id=\"" + KnowledgeMapAdmin.escapeSelector(node.id) + "\"]").removeClass("exercise-edit-hover");
        this.highlightNode(node, false);
    },

    onBadgeMouseover: function() {
        var exid = $(this).attr("data-id");
        var node = KnowledgeMapAdmin.dictNodes[exid];
        if (node) KnowledgeMapAdmin.highlightNode(node, true);
    },

    onBadgeMouseout: function() {
        var exid = $(this).attr("data-id");
        var node = KnowledgeMapAdmin.dictNodes[exid];
        if (node) KnowledgeMapAdmin.highlightNode(node, false);
    },
    ...
    onZoomChange: function(jrgNodes) {
        ...
        jrgNodes.each(function() {
            var jel = $(this);
            var node = KnowledgeMapAdmin.dictNodes[jel.attr("data-id")];

            var iconOptions = KnowledgeMapAdmin.getIconOptions(node, zoom);
            $("img", jel).attr("src", iconOptions.url);
            jel.attr("class", KnowledgeMapAdmin.getLabelClass(jel.attr("class"), node, zoom));
        });
    ...
    onIdle: function() {
        ...
        KnowledgeMapAdmin.map.panBy(0, 0);
