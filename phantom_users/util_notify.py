...
def update(user_data,user_exercise,threshold = False, isProf = False, gotBadge = False):
    ...
    if (numquest != None and numquest % 10 == 0) or (numquest != None and numquest > 20 and numquest % 5 == 0):
        UserNotifier.push_login_for_user(user,"You've answered "+str(numquest)+" questions! To save your progress you'll need to [login]")
    #Proficiency
    if isProf:
        UserNotifier.push_login_for_user(user,"You're proficient in "+str(prof)+". To save your progress you'll need to [login]")
    #First Badge
    if numbadge != None and len(numbadge) == 1 and gotBadge:
        UserNotifier.push_login_for_user(user,"Congrats on your first <a href='/profile'>badge</a>! To save your progress you'll need to [login]")
    #Every badge after
    if numbadge != None and len(numbadge) > 1 and gotBadge:
        UserNotifier.push_login_for_user(user,"You've earned <a href='/profile'>"+str(len(numbadge))+" badges</a> so far. To save your progress you'll need to [login]")
    #Every 2.5k points
    if numpoint != None and threshold:
        numpoint = 2500*(numpoint/2500)+2500
        UserNotifier.push_login_for_user(user,"You've earned over <a href='/profile'>"+str(numpoint)+ " points</a>! If you want to keep them, you'll need to [login].")
