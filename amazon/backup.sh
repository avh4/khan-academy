...
google_appengine/appcfg.py download_data --application=khanexercises --url=http://khanexercises.appspot.com/remote_api --filename=`date +%F.full.dat` --log_file=`date +%F.bulkloader.full.log` --email=kamens@gmail.com --rps_limit=15000 --http_limit=40 --bandwidth_limit=10000000 --batch_size=500 --num_threads=55 --passin < private_pw > `date +%F.full.log`
